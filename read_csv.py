...
# Path to your CSV
csv_path = 'peaks_2025-07-18_first48.csv'
...
counts = df.groupby([
    pd.Grouper(key='timestamp', freq='2min'),  # round timestamps to 15-min bins
    'ch'
]).size().reset_index(name='count')


# Optional: calculate rate per minute (divide by 2)
counts['rate_per_min'] = counts['count'] / (2)
counts['rate_per_sec'] = counts['count'] / (2*60)
...
plt.xlabel('Time')
plt.ylabel('Rate [Hz]')
plt.title('Channel Activity Rate (2 min bins)')
plt.grid(True)
plt.legend()
plt.tight_layout()
...
plt.scatter(-1.33*df[df['ch']==1]['dist_to_revsig_val'], df[df['ch']==1]['peak_heights_val'], color='blue', label=f"Ch 1", s=10, alpha=0.6)
plt.scatter(-1.33*df[df['ch']==2]['dist_to_revsig_val'], df[df['ch']==2]['peak_heights_val'], color='red', label=f"Ch 2", s=10, alpha=0.6)
plt.scatter(-1.33*df[df['ch']==3]['dist_to_revsig_val'], df[df['ch']==3]['peak_heights_val'], color='darkgreen', label=f"Ch 3", s=10, alpha=0.6)
...
all_distances_1 = -1.33*df[df['ch']==1]['dist_to_revsig_val'].explode().astype(float)
all_distances_2 = -1.33*df[df['ch']==2]['dist_to_revsig_val'].explode().astype(float)
all_distances_3 = -1.33*df[df['ch']==3]['dist_to_revsig_val'].explode().astype(float)
...
# Plot histogram
plt.figure(figsize=(8, 5))
plt.hist(df[df['ch']==1]['peak_heights_val'], bins=200, color='skyblue', edgecolor='blue', label=f"Ch 1", alpha=0.6)
plt.hist(df[df['ch']==2]['peak_heights_val'], bins=200, color='red', edgecolor='red'     , label=f"Ch 2", alpha=0.6)
plt.hist(df[df['ch']==3]['peak_heights_val'], bins=200, color='green', edgecolor='green' , label=f"Ch 3", alpha=0.6)
